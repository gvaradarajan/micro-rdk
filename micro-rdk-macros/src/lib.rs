...
//!         anyhow::bail!("unimplemented")
//!     }
//!     fn get_position(&mut self) -> anyhow::Result<micro_rdk::components::movement_sensor::GeoPosition> {
//!         anyhow::bail!("unimplemented")
//!     }
...
pub fn impl_readings_for_movement_sensor(input: TokenStream) -> TokenStream {
    ...
    let crate_ident = get_micro_rdk_crate_ident();
    let gen = quote! {
        impl #impl_generics #crate_ident::components::sensor::Readings for #name #ty_generics #where_clause {
            fn get_generic_readings(&mut self) -> anyhow::Result<#crate_ident::components::sensor::GenericReadingsResult> {
                #crate_ident::components::movement_sensor::get_movement_sensor_generic_readings(self)
            }
        }
    };
    gen.into()
...
pub fn impl_readings_for_power_sensor(input: TokenStream) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics #crate_ident::components::sensor::Readings for #name #ty_generics #where_clause {
            fn get_generic_readings(&mut self) -> anyhow::Result<#crate_ident::components::sensor::GenericReadingsResult> {
                #crate_ident::component::power_sensor::get_power_sensor_generic_readings(self)
            }
        }
    };
