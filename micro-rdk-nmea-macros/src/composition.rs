...
impl PgnComposition {
    ...
    pub(crate) fn into_token_stream(self, input: &DeriveInput) -> TokenStream2 {
        ...
        let setter_statements = self.parsing_logic;
        let attr_statements = self.attribute_getters;
        let struct_attrs = self.struct_initialization;
        let conversion_statements = self.proto_conversion_logic;
