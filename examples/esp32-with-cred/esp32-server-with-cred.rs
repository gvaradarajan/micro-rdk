...
use {
    ...
    esp_idf_svc::nvs::{EspDefaultNvsPartition, EspNvs, EspDefaultNvs},
...
fn get_str_from_nvs(viam_nvs: &EspDefaultNvs, key: &str) -> anyhow::Result<String> {
    ...
    Ok(viam_nvs.get_str(key, &mut buffer_ref)?.unwrap().trim_matches(char::from(0)).to_string())
}
...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg(not(feature = "qemu"))]
    let (ip, _wifi) = {
        let wifi = start_wifi(periph.modem, sys_loop_stack, &nvs_vars.wifi_ssid, &nvs_vars.wifi_pwd)?;
        (wifi.wifi().sta_netif().get_ip_info()?.ip, wifi)
    };

    let webrtc_certificate =
        WebRtcCertificate::new(nvs_vars.robot_dtls_cert, nvs_vars.robot_dtls_key_pair, &nvs_vars.robot_dtls_cert_fp);
...
fn start_wifi(
    ...
    sl_stack: EspSystemEventLoop, ssid: &str, password: &str
) -> anyhow::Result<Box<BlockingWifi<EspWifi<'static>>>> {
    let nvs = EspDefaultNvsPartition::take()?;
    let mut wifi = BlockingWifi::wrap(EspWifi::new(modem, sl_stack.clone(), Some(nvs.clone()))?, sl_stack)?;
