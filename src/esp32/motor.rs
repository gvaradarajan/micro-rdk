    ...
    Enc: Encoder
{
    ...
    fn get_position(&mut self) -> anyhow::Result<i32> {
        ...
        let pos_type = match props {
            EncoderSupportedRepresentations { ticks_count_supported: true, angle_degrees_supported } => {
                EncoderPositionType::TICKS
            }
            EncoderSupportedRepresentations { ticks_count_supported, angle_degrees_supported: true } => {
                EncoderPositionType::DEGREES
            }
            _ => {
                return Err(anyhow::anyhow!("encoder for this motor does not support any known position representations"));
            }
        };
