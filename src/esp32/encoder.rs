...
use super::pulse_counter::PulseCounter;

use core::ffi::{c_short, c_ulong};
use esp_idf_hal::gpio::{AnyIOPin, PinDriver, AnyInputPin};
...
use crate::common::encoder::{Encoder, EncoderPosition, EncoderPositionType, EncoderSupportedRepresentations, EncoderType};
    ...
    B: InputPin + PinExt,
{
    pub fn new(a: A, b: B) -> anyhow::Result<Self> {
        ...
        let pcnt = Box::new(PulseStorage { acc: Arc::new(AtomicI32::new(0)), unit: 0});
    ...
    pub(crate) fn from_config(cfg: ConfigType, _: Option<BoardType>) -> anyhow::Result<EncoderType> {
        match cfg {
            ConfigType::Static(cfg) => {
                let pin_a_num = match cfg.get_attribute::<i32>("a") {
                    Ok(num) => num,
                    Err(err) => {
                        return Err(anyhow::anyhow!("cannot build encoder, need 'a' pin"))
                    }
                };
                let pin_b_num = match cfg.get_attribute::<i32>("b") {
                    Ok(num) => num,
                    Err(err) => {
                        return Err(anyhow::anyhow!("cannot build encoder, need 'b' pin"))
                    }
                };
                let a = match PinDriver::input(unsafe { AnyInputPin::new(pin_a_num) }) {
                    Ok(a) => a,
                    Err(err) => return Err(anyhow::anyhow!("cannot build encoder, could not initialize pin {:?} as pin 'a'", pin_a_num))
                };
                let b = match PinDriver::input(unsafe { AnyInputPin::new(pin_b_num) }) {
                    Ok(b) => b,
                    Err(err) => return Err(anyhow::anyhow!("cannot build encoder, could not initialize pin {:?} as pin 'b'", pin_b_num))
                };
    ...
    pub fn setup_pcnt(&mut self) -> anyhow::Result<()> {
        ...
        esp!(unsafe {
            esp_idf_sys::pcnt_isr_handler_add(
                ...
                // Box::new(self.pulse_counter) as *mut core::ffi::c_void
    ...
    fn get_position(
        ...
        match position_type {
            EncoderPositionType::TICKS | EncoderPositionType::UNSPECIFIED => {
                ...
                let pos = EncoderPositionType::TICKS.wrap_value(count as f32);
                // println!("encoder pos type val: {:?}", pos.value);
