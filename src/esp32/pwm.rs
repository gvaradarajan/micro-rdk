...
use esp_idf_sys::{ledc_get_freq, ledc_timer_t, ledc_channel_t_LEDC_CHANNEL_MAX, ledc_set_freq, ledc_bind_channel_timer, EspError};
...
pub(crate) fn create_pwm_driver(pin: AnyIOPin, starting_frequency_hz: u32) -> Result<PwmDriver<'static>, Esp32PwmError> {
...
impl<'a> PwmDriver<'a> {
    fn new(pin: AnyIOPin, channel: PwmChannel, starting_frequency_hz: u32) -> Result<PwmDriver<'a>, Esp32PwmError> {
        let mut ledc_manager = LEDC_MANAGER.lock().unwrap();
        let timer_number = ledc_manager.take_timer(starting_frequency_hz, channel)?;
        let ledc_driver = get_ledc_driver_by_channel(channel, timer_number, starting_frequency_hz, pin)?;
    ...
    pub fn get_timer_frequency(&self) -> u32 {
        unsafe { ledc_get_freq(SpeedMode::LowSpeed.into(), self.ledc_driver.timer()) }
    }
...
impl LedcManager {
    ...
    fn find_timer_by_frequency(&mut self, frequency_hz: u32) -> Option<usize> {
        self.timer_frequencies.iter().position(|&x| x == Some(frequency_hz))
    }

    fn take_timer(&mut self, frequency_hz: u32, channel: PwmChannel) -> Result<usize, Esp32PwmError> {
    ...
    fn take_channel(&mut self, n: i32, pin: i32) -> Result<PwmChannel, Esp32PwmError> {
        ...
        let res = match n {
            ...
            6 => PwmChannel::C6,
            7 => PwmChannel::C7,
            _ => { return Err(Esp32PwmError::ChannelNotFound(n)); },
        };
